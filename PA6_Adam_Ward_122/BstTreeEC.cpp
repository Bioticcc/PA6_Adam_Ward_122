...
template <typename T>
BstTreeEC<T>::BstTreeEC() : inputName("morseLibrary.txt"), root(nullptr) {
	makeBST();
}

template <typename T>
BstNodeEC<T>* BstTreeEC<T>::insertToTree(BstNodeEC<T>* root, T text, string morse) {
	//essentially this versions head ptr: root

	if (root == nullptr) {
		//means its the initial root, so 'M'? i think? bababooey cause hell if i know tbh
		return new BstNodeEC(text, morse);
...
template <typename T>
BstNodeEC<T>* BstTreeEC<T>::readTree(string inputName, BstNodeEC<T>* root) {
...
template <typename T>
void BstTreeEC<T>::print(BstNodeEC<T>* root) {
	//root = getRoot();
	if (!root) return;
	print(root->Lchi);
...
template <typename T>
string BstTreeEC<T>::sift(BstNodeEC<T>* root, char target) {
...
template <typename T>
void BstTreeEC<T>::makeBST() {
	BstTreeEC* root = nullptr;
...
template <typename T>
string BstTreeEC<T>::readInput(string inputName) {
	std::ifstream input;
	input.open(inputName, std::ios::in);
	string text;
	string test1;
...
template <typename T>
void BstTreeEC<T>::search(BstNodeEC<T>* root) {
	string input;
...
template <typename T>
int BstTreeEC<T>::deleteTree(BstNodeEC<T>* root) {
	if (root != nullptr) {
